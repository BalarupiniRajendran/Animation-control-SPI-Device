...
sequence led_seq[] = { 0x0F, 0x00, 0x0C, 0x01, 0x09, 0x00, 0x0A, 0x0F, 0x0B, 0x07, 0x01, 0x06,
                     0x02, 0x06, 0x03, 0x06, 0x04, 0x83, 0x05, 0x7F, 0x06, 0x24, 0x07, 0x22,
                     0x08, 0x63};
sequence led_seqrr[] = { 0x0F, 0x00, 0x0C, 0x01, 0x09, 0x00, 0x0A, 0x0F, 0x0B, 0x07, 0x01, 0x06,
		               0x02, 0x06, 0x03, 0x06, 0x04, 0x03, 0x05, 0x7F, 0x06, 0xA2, 0x07, 0x32,
		               0x08, 0x16};
sequence led_seql[] = { 0x0F, 0x00, 0x0C, 0x01, 0x09, 0x00, 0x0A, 0x0F, 0x0B, 0x07, 0x01, 0x60,
		              0x02, 0x60, 0x03, 0x60, 0x04, 0xC1, 0x05, 0xFE, 0x06, 0x24, 0x07, 0x44,
		              0x08, 0xC6};
sequence led_seqlr[] = {  0x0F, 0x00, 0x0C, 0x01, 0x09, 0x00, 0x0A, 0x0F, 0x0B, 0x07, 0x01, 0x60,
		                0x02, 0x60, 0x03, 0x60, 0x04, 0xC0, 0x05, 0xFE, 0x06, 0x45, 0x07, 0x4C,
		                0x08, 0x68};
...
void* display_function(void* arg){
    ...
    if(fd_spi==-1)
    {
        ...
        while(1){
            ...
            if(dir ==1){
                tr_array[0] = led_seq [i];
                tr_array[1] = led_seq [i+1];
            }
            else if(dir==2){
                tr_array[0] = led_seqrr [i];
                tr_array[1] = led_seqrr [i+1];
            }
            else if(dir==3){
                tr_array[0] = led_seql [i];
                tr_array[1] = led_seql [i+1];
            }
            else if(dir==4){
                tr_array[0] = led_seqlr [i];
                tr_array[1] = led_seqlr [i+1];
            }
